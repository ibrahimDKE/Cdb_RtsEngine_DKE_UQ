...
import cdb.AggregateGroupByView;
import cdb.AggregateView;
import cdb.CardinalityView;
import cdb.RowSampleView;
import cdb.View;
...
public class QueryExecutor {
	...
	public List<View> execute(List<DifferenceQuery> optimizedQueries,
		...
		for (DifferenceQuery optQuery : optimizedQueries) {
                    //Added by Himos
                    if(this.settings.extime>0){
                            sstop=System.currentTimeMillis() - start;
                            if(sstop>this.settings.extime) break;
                    }
                            ////
		...
		Utils.writeToFile(logFile, "Executor: " + (System.currentTimeMillis() - start));
                Utils.writeToFile(logFile, "Time limit: " + (this.settings.extime));
	...
	public void executeQuery(DifferenceQuery optQuery, String query, 
		...
		while (rs.next()) {
			for (int i = 1; i < columnCount + 1; i++ ) {
				...
				if (columnName.endsWith("count") || columnName.endsWith("sum")|| columnName.endsWith("max")||columnName.endsWith("min")) {
			...
			for (DifferenceQuery dq : optQuery.derivedFrom) { // for each query whose result this query contributes to
				...
				for (String attr : aggAttrs) {
					attr = attr.toLowerCase();
                                        AggregateFunctions func= null;
					if (attr.endsWith("count")) { func= AggregateFunctions.COUNT;}
                                        if (attr.endsWith("sum")) { func= AggregateFunctions.SUM;}
                                        if (attr.endsWith("max")) { func= AggregateFunctions.MAX;}
                                        if (attr.endsWith("min")) { func= AggregateFunctions.MIN;}
	...
	private class ExecuteParallelQueryWithTempTables implements Runnable {
		...
		public void run() {
			...
			//System.out.println(optSQLQuery.get(0)); Himos
			tempTableCreationTime += System.currentTimeMillis() - start;
